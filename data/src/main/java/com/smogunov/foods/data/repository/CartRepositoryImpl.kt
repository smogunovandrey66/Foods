...
class CartRepositoryImpl(
    ...
    private val _resultChangeCartItem: MutableStateFlow<ResultData> =
    ...
    override suspend fun loadCartItems() {
        try {
                ...
                dishWithCartItem.dishDB.let { dishDB ->
                    CartItem(
                        Dish(
                            ...
                            dishDB.description,
                            dishDB.imageUrl
                        ), dishWithCartItem.cartItem.count
                    )
                }
            ...
            _cartItems.value =
                ResultData.Error(e.message ?: "error load cartItems[${e.javaClass.name}]")
        }
    }
    ...
    override suspend fun changeCartItem(idDish: Int, count: Int) {
        try {
            ...
            _resultChangeCartItem.value =
                ResultData.Error(e.message ?: "error change cartItem[${e.javaClass.name}]")
        }
    }
}